...
${renderedHtml}
    ...
    <script type="module">
        ...
        document.querySelectorAll('pre.mermaid').forEach((el, i) => {
            ...
            el.setAttribute('data-mermaid-id', 'mermaid-' + i);
        });
        ...
        window.__reRenderMermaid = async function(theme) {
            ...
            document.querySelectorAll('pre.mermaid').forEach(el => {
                ...
                if (src) {
                    // Reset the element so mermaid re-processes it
    ...
    <script>
        ...
        toggleButton.addEventListener('click', () => {
            if (htmlElement.getAttribute('data-theme') === 'dark') {
                ...
                localStorage.setItem('theme', 'light');
                toggleButton.textContent = '🌙';
                if (window.__reRenderMermaid) window.__reRenderMermaid('default');
            } else {
                htmlElement.setAttribute('data-theme', 'dark');
                localStorage.setItem('theme', 'dark');
                toggleButton.textContent = '☀️';
                if (window.__reRenderMermaid) window.__reRenderMermaid('dark');
            }
        });
